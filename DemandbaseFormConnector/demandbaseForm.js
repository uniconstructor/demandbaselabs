...
l.status){if(l.sequence>f.latest_result.sequence||!f.latest_result.sequence)f.latest_result=l;try{f.response_callback&&"function"===typeof f.response_callback&&f.response_callback(f.latest_result)}catch(m){console.log("response callback error: ",m)}p(k.map(f.latest_result.picks,function(m){var p=f.label;!f.label.indexOf("{state}"!==-1)||(m.state&&isNaN(m.state)||!m.country)||(p=f.label.replace("{state}","{country}"));return{label:k.nano("function"==typeof f.label?f.label.apply(this,[k,m]):p,m),value:e.term,
ip:l.ip,pick:m}}))}else f.log("ERROR: ",l.error),p([]),f.textField.autocomplete("destroy")},error:function(e,k,l){f.log(k,l);p([]);f.textField.autocomplete("destroy")}})},filter:function(e,p){if("function"!==typeof p)throw new TypeError;for(var f=e.length>>>0,k=[],m=0;m<f;m++)if(m in e){var l=e[m];p.call(e,l,m,e)&&k.push(l)}return k},_change:function(e,p){if(!p.item){var f=Demandbase.CompanyAutocomplete,k=f.jQuery,m=void 0,l=k.trim(k(this).val()),q={input_match:{}},v={key:f.key,input:f.textField.val(),
email:k("#"+f.email).val(),timezone:-(new Date).getTimezoneOffset()/60,session:f.session,sequence:f.sequence};try{v.domain=k("#"+f.email).val().split("@").pop()}catch(x){}k.getJSON(Demandbase.host+"/analysis/autocomplete/"+f.version+"/log.json?callback=?",v,function(e){});try{if(0<l.length){f.latest_result&&f.latest_result.picks&&0<f.latest_result.picks.length&&(m=f.filter(f.latest_result.picks,function(e){return e.company_name.match(RegExp("^"+l,"i"))})[0])&&(q={input_match:m});try{f.callback(q)}catch(w){f.errors.push(w),
f.log("Demandbase.CompanyAutocomplete:","ERROR:","An error occurred in the callback"),f.log(w)}}}catch(u){f.errors.push(u),f.log("Demandbase.CompanyAutocomplete:","ERROR:","Unable to detect input match: ",u)}}}};
